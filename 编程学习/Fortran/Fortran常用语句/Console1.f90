...
write(number,'(f7.3)') pi  ! 用write可以把数字类型转成字符类型。'(f7.3)'是输出浮点数的格式，如果用*来代替，字符串的长度需要够长猜想。整型格式用类似'(i3)'这样
...
write(*,*) '----矩阵本征矢和本征值----'
call geev(A=dd, W=eigenvalues, VR=eigenvectors, INFO=info)  ! 这里A矩阵最好用上复数，W是本征值一维数组，VR是本质矢二维数组，都是复数。INFO是整数。
