...
aa=1   %没加分号“;”,默认打印输出
bb=2;  %加了分号“;”，即不打印输出
cc1=zeros(2,3)  %零矩阵用zeors()
cc2=eye(3,3)  %单位矩阵

%矩阵乘积
matrix1=[3,3;3,3]  %矩阵的书写格式是这样的，里面分号代表矩阵换一行。下标是从1开始记。
matrix2=[2,0;0,2]
matrix_product_1=matrix1*matrix2  % *是正常的矩阵乘积
matrix_product_2=matrix1.*matrix2  % .*是矩阵每个元素对应相乘

%循环
for i0=1:0.5:2  %循环内容为for到end。a:b:c代表最小为a，最大为c，步长为b，区间为左闭右闭[a,c]
