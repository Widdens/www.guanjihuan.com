...
def main():
    ...
    dim = h0.shape[0]
    n = 4  # 选取第n个能级
    eigenvalue, eigenvector = np.linalg.eig(h0)
    # print(h0)
    ...
    print('\n波函数模的平方：', np.square(np.abs(eigenvector[:, n-1])))   # 查看第n个能级对应的波函数模的平方
    green = np.linalg.inv((eigenvalue[n-1]+1e-15j)*np.eye(dim)-h0)  # 第n个能级对应的格林函数
    total = np.trace(np.imag(green))  # 求该能级格林函数的迹，对应的是总态密度（忽略系数）
    print('归一化后的态密度分布：')
    for i in range(dim):
